...
def PhenoApp(m=None):
    ...
    eelter_object = ee.FeatureCollection('projects/ee-digdgeografo/assets/elter_full_phen_big')
    eedonana_object = eelter_object.filter(ee.Filter.eq('name', 'Doñana Long-Term Socio-ecological Research Platform'))
    eebaixo_object = eelter_object.filter(ee.Filter.eq('name', 'Baixo Sabor LTER'))
    eeeisenwurzen_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Platform Eisenwurzen (EW)'))
    eeCairngorms_object = eelter_object.filter(ee.Filter.eq('name', 'Cairngorms National Park LTSER'))
    eeVenice_object = eelter_object.filter(ee.Filter.eq('name', 'IT16-Lagoon of Venice'))
    eeArmorique_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Zone Atelier Armorique'))
    eeRhine_object = eelter_object.filter(ee.Filter.eq('name', 'Rhine-Main-Observatory'))
    eeMinho_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Minho Estuary'))
    eeNeusiedler_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Platform Neusiedler See - Seewinkel'))
    ...
    eLTER_SITES = {'Baixo Sabor': 
                    ...
                    'Minho Estuary':
                    ['LTSER Minho Estuary - Portugal', eeMinho_object],
                    'Neusiedler See':
                    ['LTSER Platform Neusiedler See - Seewinkel - Austria', eeNeusiedler_object],
                    'Atelier-Alpes':
                    ['LTSER Zone Atelier Alpes - Francea', eeAtelier_object]
                    }
    ...
    def submit_clicked(b):
        ...
        if m is not None and collection.value == "MODIS MCD12Q2.006":
            ...
            base = 'projects/ee-digdgeografo/assets/Donana_{}_{}{}'.format(start_year.value, keyPhen, str(phenometrics_val.value)[0])
            nbase = ee.Image(base)
            #print(base)
            #geom = eLTER_SITES[site.value][1]   
            #dataset = dataset[collection.value].filterBounds(geom).filterDate(start_date, end_date)
